...
When hitting `Enter`, a new paragraph will be created. A hard line break can be added by pressing Shift+Enter, which will cause subsequent text to appear on a new line but within the same paragraph. On export, the line break character will be the value specified by the `mark-sharp.formatting.lineBreakStyle` setting, with the default being a backslash `\` character.
...
| mark-sharp.display.documentWidth                | Controls the page width of the editor. Choose `letter` for a fixed A4-sized layout, or `fullPage` to span the full width of the viewport. | fullPage                                                                                             |
...
| mark-sharp.formatting.lineBreakStyle            | The line break style to use when saving the document back to the markdown file.                      | The style parsed from the markdown file is used when importing, and a backslash is used for new line breaks created when editing. |
